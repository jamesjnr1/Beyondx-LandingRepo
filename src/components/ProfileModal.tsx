...
export default function ProfileModal({
  ...
  const pickPhoto = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (f) setP((prev) => ({ ...prev, avatar: URL.createObjectURL(f) }))
  }
        ...
        <div className="mb-5 flex items-center gap-4">
          ...
          <div>
            <button type="button" onClick={() => fileRef.current?.click()}
              ...
              <Camera size={15} aria-hidden="true" /> {p.avatar ? 'Change photo' : 'Add photo'}
            </button>
            <input ref={fileRef} type="file" accept="image/*" onChange={pickPhoto} className="sr-only" aria-label="Upload profile picture" />
